...
public class ExampleServlet extends HttpServlet
{
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException
	{
		...
		log.info(String.format(	"question_id: %s with value %s and preferred_medium %s",
								questionId[0], value[0], preferred_medium[0]));

		resp.getOutputStream()
			.println(	answerQuestion(value[0], questionId[0], preferred_medium[0]).getEntity()
																					.toString());
	}

	public Response answerQuestion(String answer_json, String answerId, String preferred_medium)
	{
		cAskFast = new AskFast(getUrl());
		if (answerId.equals("1"))
		{
			cAskFast.ask("Are you coming to my bday party?", QuestionType.CLOSED, null);
			cAskFast.addAnswer(cSampleAnswers.get(0), getUrl() + "?question_id=10");
			cAskFast.addAnswer(cSampleAnswers.get(1), getUrl() + "?question_id=11");
		}
	...
	{
		cAskFast.ask("Any plans for the day?", QuestionType.CLOSED, null);
		cAskFast.addAnswer(cSampleAnswers.get(0), getUrl() + "?question_id=10");
		cAskFast.addAnswer(cSampleAnswers.get(1), getUrl() + "?question_id=11");
